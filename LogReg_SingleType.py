...
data = data[data["option_call_or_put"] == "CALLS"]
...
logreg = LogisticRegression(penalty='l2', random_state=1337)
#ada.fit(train_data, train_labels, sample_weight=compute_sample_weight(class_weight="balanced", y=train_labels))
logreg.fit(train_data, train_labels, sample_weight=compute_sample_weight(class_weight=class_weight, y=train_labels))
#ada.fit(train_data, train_labels)
# STEP 2 Errors
print("TRAINING ACCURACY: "+str(logreg.score(train_data, train_labels)))
print("TESTING ACCURACY: "+str(logreg.score(test_data, test_labels)))
